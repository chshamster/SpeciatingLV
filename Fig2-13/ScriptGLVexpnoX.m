%This script generates Figs. 2-13 from
...
%section 2. To computes averages such as Fig.3, this script must be run
%multiple times. For Fig. 5, the script must be run multiple times for
...
eta = 0.03;                      % evolutionary noise parameter when creating new species
...
ClusterDistPphen=kmeans(full(P),5,'Distance','sqeuclidean','Replicates',100);
...
%silhoutette. 5 is the number of cluster it is forced to find
ClusterDistRphen=kmeans(beta*full(P'),5,'Distance','sqeuclidean','Replicates',100);
