%This script generates Fig. 2a and Fig. 2b from
...
Tvector=0;  %vector that will store all t values
...
while told<Tend && max(NtotalP,NtotalR)<expecnumspec/2
    ...
    if j<expecnumspec/2+1 %update A for new species if ancestor is prey
        xnew(NtotalP+1)=frac*xnew(j); xnew(j)=(1-frac)*xnew(j);
        ...
        xnew(expecnumspec/2+NtotalR+1)=frac*xnew(j); xnew(j)=(1-frac)*xnew(j);
